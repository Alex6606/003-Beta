...
# ============================================
"""
Módulo de generación automática de features técnicos.
Combina los indicadores del módulo indicators.py aplicados a distintas ventanas.
"""

import numpy as np
import pandas as pd
from indicators import *  # importa todas las funciones base (rsi, atr, bollinger, etc.)
from indicators import _roc, _rma, _logret_std, _rsi, _stoch_k_smooth, _williams_r_0_100, _cci, _trix, _dist_to_ma, \
    _atr_with_method, _bollinger, _keltner_bandwidth, _adx_plus_minus_di, _obv, _vol_roc, _mfi, _cmf


# ===================== Generador AUTO por ventana =====================
def make_features_auto(
    ...
    """
    Genera automáticamente un conjunto de features basadas en indicadores técnicos
    aplicados a diferentes ventanas de tiempo.

    Retorna:
        - features_df : DataFrame con todas las columnas generadas.
        - feature_types : dict con el tipo de escala de cada feature (para normalización).
        - combos_usados : dict con los indicadores generados por familia.
    """
    need = {'Open', 'High', 'Low', 'Close', 'Volume'}
    if not need.issubset(data.columns):
        raise ValueError(f"Faltan columnas requeridas: {sorted(need - set(data.columns))}")
